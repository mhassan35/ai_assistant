...
export default function ChatPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden mt-16">
      ...
      `}>
        <button className="bg-purple-700 hover:scale-95 duration-300 hover:bg-purple-800 active:bg-purple-900 text-gray-300 rounded-lg p-2.5 sm:p-3 mb-4 flex items-center justify-center gap-2 text-sm sm:text-base">
          <FiPlus /> New Chat
        </button>
        ...
        <div className="relative mb-4 sm:mb-6">
          ...
          <input
            ...
            className="w-full bg-[#161f2f] text-gray-300 rounded-lg pl-9 p-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-1 sm:space-y-2">
          {chatHistory.map((chat, index) => (
            ...
            >
              <h3 className="text-gray-300 text-xs sm:text-sm truncate">{chat.title}</h3>
              <p className="text-gray-300 text-xs mt-0.5">{chat.date}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col w-full">
        <div className="flex justify-end p-2 sm:p-4 gap-2 sm:gap-4">
          ...
          <button className="p-1.5 sm:p-2 hover:bg-slate-200 rounded-full">
            <FiShare className="text-lg sm:text-xl text-gray-300" />
          </button>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center p-3 sm:p-4">
          <div className="text-center w-full max-w-[90%] sm:max-w-[80%]">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-gray-300 mb-2">Welcome to DevAssist</h2>
            <p className="text-sm sm:text-base text-purple-400 mb-6 sm:mb-8">Your Coding AI Assistant</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 max-w-4xl mx-auto px-2 sm:px-4">
              {['Debug your code', 'Learn new concepts', 'Get best practices'].map((text, i) => (
                <div key={i} className="bg-gray-300 p-3 sm:p-4 rounded-xl shadow-sm hover:shadow-md transition-shadow cursor-pointer">
        ...
        <div className="p-2 sm:p-4 md:p-6">
          <div className="max-w-3xl mx-auto relative">
            ...
            <div className="absolute right-1.5 sm:right-2 top-1/2 -translate-y-1/2 flex gap-1 sm:gap-2">
              {/* <button className="p-1.5 sm:p-2 hover:bg-gray-300 rounded-full hidden sm:block">
                <FiMic className="text-lg sm:text-xl text-[#020617]/95" />
              </button> */}
              <button className="bg-purple-700 text-gray-300 rounded-full p-1.5 sm:p-2">
